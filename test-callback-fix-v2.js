...
console.log('🧪 Teste Callback-Fix lokal (Version 2)...');
  ...
  async triggerGet() {
    ...
    for (let i = 0; i < this.handlers.length; i++) {
      ...
      try {
        ...
        // Warte auf asynchrone Callbacks
...
async function runTest() {
  ...
  await accessory.mainService.getCharacteristic('CurrentAmbientLightLevel').triggerGet();
  await accessory.mainService.getCharacteristic('CurrentAmbientLightLevel').triggerGet();
  await accessory.mainService.getCharacteristic('CurrentAmbientLightLevel').triggerGet();
